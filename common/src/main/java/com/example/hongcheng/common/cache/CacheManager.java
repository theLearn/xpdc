package com.example.hongcheng.common.cache;
...
public class CacheManager {
	...
	// 获取缓存信息
	public static <T> T getCacheObj(String key, Class<T> t) {
		if (hasCache(key)) {
			Cache cache = getCache(key);
			if (cacheExpired(cache)) { // 调用判断是否终止方法
				cache.setExpired(true);
			}
			if(cache != null && cache.getValue() != null && cache.getValue().getClass().equals(t)){
