...
public final class CityData {
    ...
    static String[] hotCity = new String[]{"北京", "上海", "广州", "深圳", "武汉", "杭州", "重庆"};

    private List<CityItem> all = new ArrayList<CityItem>();
    private List<CityItem> hot = new ArrayList<>();
    private List<Pair<String, List<CityItem>>> allSource = new ArrayList<>();

    private CityData(){}

    private static class CityDataSingleHolder{
        static CityData INSTANCE = new CityData();
    }

    public static CityData getInstance() {
        return CityDataSingleHolder.INSTANCE;
    }
    ...
    public void init() {
        ...
        for (CityItem cityItem : list) {
            ...
            if (currentTitle.equals(lastTitle)) {
                ...
                temp.add(cityItem);
                allSource.add(Pair.create(currentTitle, temp));
            }
        }
    }
     ...
     * a-z排序
     */
    private Comparator comparator = new Comparator<CityItem>() {
        @Override
