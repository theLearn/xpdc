 ...
 */
public final class CustomInertiaTimerTask extends TimerTask {
     ...
     */
    public CustomInertiaTimerTask(CustomWheelView wheelView, float velocityY) {
        super();
    ...
    public final void run() {
        ...
        if (Math.abs(mCurrentVelocityY) >= 0.0F && Math.abs(mCurrentVelocityY) <= 20F) {
            ...
            mWheelView.getHandler().sendEmptyMessage(CustomMessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
        ...
        //刷新UI
        mWheelView.getHandler().sendEmptyMessage(CustomMessageHandler.WHAT_INVALIDATE_LOOP_VIEW);
    }
}
