...
public class CustomWheelView extends View {
    ...
    private void initLoopView(Context context) {
        ...
        handler = new CustomMessageHandler(this);
        gestureDetector = new GestureDetector(context, new CustomLoopViewGestureListener(this));
    ...
    public void smoothScroll(ACTION action) {//平滑滚动的实现
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new CustomSmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    public final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new CustomInertiaTimerTask(this, velocityY), 0, VELOCITY_FLING, TimeUnit.MILLISECONDS);
    }
