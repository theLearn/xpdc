...
public class WheelView extends View {
    ...
    private void init(Context context) {
        ...
        mPaint.setColor(getResources().getColor(R.color.twentyalpha));
        mPaint.setTextSize(spTopx(context, 18));

        selectPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        selectPaint.setStyle(Style.FILL);
        selectPaint.setTextAlign(Align.CENTER);
        selectPaint.setColor(getResources().getColor(R.color.lightseagreen));
        ...
        centerLinePaint.setColor(getResources().getColor(R.color.twentyalpha));
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
        mPaint.setAlpha(255 * (SHOW_SIZE - position + 1) / SHOW_SIZE);
...
//                canvas.drawLine(0, itemHeight * 2, width, itemHeight * 2, centerLinePaint);
    ...
    private void doMove(MotionEvent event) {
        ...
        mMoveLen += 2 * (event.getY() - mLastDownY);
